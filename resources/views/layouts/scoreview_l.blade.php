        ...
        @if (isset($music_data->sp_l_lv))
            <tr>
                ...
                <td class="ctype_hide">
                    ...
                    @elseif ($score_datasp_l_score?->sp_l_cleartype == 'FULLCOMBO CLEAR')
                        <span
                ...
                <td style="text-align: center;line-height: 0.9;">
                    ...
                    <br><span style="font-size: 10px;">
                        ...
                        @elseif ($score_data?->sp_l_djlevel == 'A')
                            AA -{{ ceil($music_data?->{'sp_b_notes'} * 2 * 0.834) - $score_data?->sp_l_score }}
                        @elseif ($score_data?->sp_l_djlevel == 'AA' && $score_data?->sp_l_score < $music_data?->{'sp_b_notes'} * 2 * 0.889)
                            AA +{{ ceil($score_data?->sp_l_score - $music_data?->{'sp_b_notes'} * 2 * 0.834) }}
                        @elseif ($score_data?->sp_l_djlevel == 'AA')
                            AAA {{ ceil($music_data?->{'sp_b_notes'} * 2 * 0.889) - $score_data?->sp_l_score }}
                        @elseif ($score_data?->sp_l_djlevel == 'AAA' && $score_data?->sp_l_score < $music_data?->{'sp_b_notes'} * 2 * 0.944)
                            AAA +{{ ceil($score_data?->sp_l_score - $music_data?->{'sp_b_notes'} * 2 * 0.889) }}
                        @elseif ($score_data?->sp_l_djlevel == 'AAA' && $score_data?->sp_l_score > $music_data?->{'sp_b_notes'} * 2 * 0.944)
                            MAX {{ ceil($music_data?->{'sp_b_notes'} * 2) - $score_data?->sp_l_score }}
                        @else
                            ---
                        @endif
                    </span>
                        ...
                        <div class="rate_wrapper">
                            ...
                            <div class="rate_graph">
                                <div class="bar_area">
                                    <div class="bar my"
                                        style="flex-basis:
										@if ($score_data?->sp_l_score == 0 || $music_data?->{'sp_b_notes'} == 0) 0%
										@else
											{{ round(($score_data?->sp_l_score / ($music_data?->{'sp_b_notes'} * 2)) * 100, 2) }}% @endif">
                                    </div>
                                </div>
                            </div>
                            <p>
                                @if ($score_data?->sp_l_score == 0 || $music_data?->{'sp_b_notes'} == 0)
                                    0 %
                                    @else{{ round(($score_data?->sp_l_score / ($music_data?->{'sp_b_notes'} * 2)) * 100, 2) }}%
                                @endif
                            </p>
                        </div>
                    </div>
